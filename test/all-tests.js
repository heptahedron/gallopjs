...
/***/ function(module, exports, __webpack_require__) {
	...
	var _gllpc = __webpack_require__(38);

	var gllpc = _interopRequireWildcard(_gllpc);
	...
	describe('gllpc', function () {
	  ...
	  var basicExpectations = [{ pClass: gllpc.StringLiteralParser,
	    tests: [{ p: bird, str: 'bird', succ: { val: 'bird', rem: '' },
	      ...
	      it: 'should fail to parse an incorrect string' }, { p: empty, str: '', succ: { val: '', rem: '' },
	      it: 'should succeed in parsing the empty string' }, { p: empty, str: 'extra', succ: { val: '', rem: 'extra' },
	      it: 'should not consume anything when parsing the empty string' }] }, { pClass: gllpc.TerminalSequentialParser,
	    tests: [{ p: simpleSeq, str: 'birdistheword',
	      succ: { val: ['bird', 'istheword'], rem: '' },
	      it: 'should successfully parse both terminals in sequence' }, { p: simpleSeq, str: 'bird', fail: { rem: '' },
	      it: 'should fail to parse a too-short string' }, { p: simpleSeq, str: 'istheword', fail: { rem: 'istheword' },
	      it: 'should not forget to parse the first terminal' }, { p: simpleSeq, str: '', fail: { rem: '' },
...
/***/ function(module, exports) {
	...
	var ABSTRACT_METHOD = 'Abstract method called!';

	var Result = exports.Result = function Result(val, rem) {
	...
	var Success = exports.Success = function (_Result) {
	...
	var Failure = exports.Failure = function (_Result2) {
	  ...
	  _createClass(Trampoline, [{
	    ...
	    value: function run() {
	      ...
	      var _loop = function _loop() {
	        ...
	        var _dispatchStack$pop = _this3.dispatchStack.pop();

	        var _dispatchStack$pop2 = _slicedToArray(_dispatchStack$pop, 2);

	        var parser = _dispatchStack$pop2[0];
	        var stream = _dispatchStack$pop2[1];
	        ...
	        parser.chain(_this3, stream, function (res) {

	          if (!_this3.results.has(stream)) {
	            _this3.results.set(stream, new Map());
	          }
	          if (!_this3.results.get(stream).has(parser)) {
	            _this3.results.get(stream).set(parser, new Set());
	          }
	          if (res instanceof Success) {
	            _this3.results.get(stream).get(parser).add(res);
	          }

	          _this3.resultCallbacks.get(stream).get(parser).forEach(function (resultCallback) {
	    ...
	    key: 'add',
	    value: function add(parser, stream, resultCallback) {
	       ...
	       */
	      if (this.results.has(stream) && this.results.get(stream).has(parser)) {
	...
	var Parser = exports.Parser = function () {
	  ...
	  _createClass(Parser, [{
	    ...
	    key: 'alternately',
	    value: function alternately(alternate) {
	...
	var TerminalParser = exports.TerminalParser = function (_Parser) {
	  _inherits(TerminalParser, _Parser);

	  function TerminalParser() {
	    _classCallCheck(this, TerminalParser);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(TerminalParser).apply(this, arguments));
	  }

	  _createClass(TerminalParser, [{
	    key: 'followedBy',
	    value: function followedBy(next) {
	    ...
	    key: 'chain',
	    value: function chain(trampoline, stream, resultCallback) {
	...
	var NonTerminalParser = exports.NonTerminalParser = function (_Parser2) {
	  _inherits(NonTerminalParser, _Parser2);

	  function NonTerminalParser() {
	    _classCallCheck(this, NonTerminalParser);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(NonTerminalParser).apply(this, arguments));
	  }

	  _createClass(NonTerminalParser, [{
	    key: 'parse',
	    value: function parse(stream) {
	      var trampoline = new Trampoline(),
	          results = [];
	      this.chain(trampoline, stream, function (res) {
	...
	var StringLiteralParser = exports.StringLiteralParser = function (_TerminalParser) {
	  _inherits(StringLiteralParser, _TerminalParser);

	  function StringLiteralParser(str) {
	    _classCallCheck(this, StringLiteralParser);

	    var _this6 = _possibleConstructorReturn(this, Object.getPrototypeOf(StringLiteralParser).call(this));

	    _this6.str = str;
	    return _this6;
	  }

	  _createClass(StringLiteralParser, [{
	    key: 'parse',
	    value: function parse(stream) {
	      // not *really* a stream, but you know
	      if (stream.length < this.str.length) {
	        return new Failure('Unexpected end of stream.', stream);
	      }

	      var recvdStr = stream.substring(0, this.str.length);
	      if (recvdStr === this.str) {
	        return new Success(this.str, stream.substr(this.str.length));
	      } else {
	        return new Failure('Expected \'' + this.str + '\', but got \'' + recvdStr + '\'.', stream);
	      }
	    }
	  }]);
	...
	var TerminalSequentialParser = exports.TerminalSequentialParser = function (_TerminalParser2) {
	  _inherits(TerminalSequentialParser, _TerminalParser2);

	  function TerminalSequentialParser() {
	    _classCallCheck(this, TerminalSequentialParser);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(TerminalSequentialParser).apply(this, arguments));
	  }

	  _createClass(TerminalSequentialParser, [{
	    key: 'parse',
	    value: function parse(stream) {
	      var res1 = this.first.parse(stream);
	      if (res1 instanceof Success) {
	        var res2 = this.next.parse(res1.rem);
	  ...
	  return TerminalSequentialParser;
	}(TerminalParser);

	var NonTerminalSequentialParser = exports.NonTerminalSequentialParser = function (_NonTerminalParser) {
	  _inherits(NonTerminalSequentialParser, _NonTerminalParser);

	  function NonTerminalSequentialParser() {
	    _classCallCheck(this, NonTerminalSequentialParser);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(NonTerminalSequentialParser).apply(this, arguments));
	  }

	  _createClass(NonTerminalSequentialParser, [{
	    key: 'chain',
	    value: function chain(trampoline, stream, resultCallback) {
	      var _this9 = this;

	      this.first.chain(trampoline, stream, function (res1) {
	        if (res1 instanceof Success) {
	          _this9.next.chain(trampoline, res1.rem, function (res2) {
	            if (res2 instanceof Success) {
	  ...
	  return NonTerminalSequentialParser;
	}(NonTerminalParser);

	var DisjunctiveParser = exports.DisjunctiveParser = function (_NonTerminalParser2) {
	  _inherits(DisjunctiveParser, _NonTerminalParser2);

	  function DisjunctiveParser() {
	    _classCallCheck(this, DisjunctiveParser);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(DisjunctiveParser).apply(this, arguments));
	  }

	  _createClass(DisjunctiveParser, [{
	    key: '_possibleParsesOf',
	    value: function _possibleParsesOf(parser, seen) {
	      if (seen.has(parser)) {
	        return [];
	      }
	    ...
	    key: '_gatherPossible',
	    value: function _gatherPossible(seen) {
	      // consider caching possibilities
	      // but would impact runtime alteration of parsers

	      return this._possibleParsesOf(this.first, seen).concat(this._possibleParsesOf(this.next, seen));
	    }
	  }, {
	    key: 'chain',
	    value: function chain(trampoline, stream, resultCallback) {
	      ...
	      try {
	        for (var _iterator = this._gatherPossible(new Set())[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	          var possibleParser = _step.value;

	          trampoline.add(possibleParser, stream, function (res) {
	            /**
